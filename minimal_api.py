...
def process_real_data(items):
    ...
    try:
        ...
        for item in items:
            try:
                # Check if this is a habit record (SK starts with DAILY# for daily performance)
                sk = item.get('SK', '')
                if not sk.startswith('DAILY#'):
                    continue
                
                # Extract habit information from the actual DynamoDB structure
                habit_name = item.get('habit_name', 'Unknown Habit')
                category = item.get('category', 'Uncategorized')
                
                # Calculate success rate from completed status
                completed = item.get('completed', False)
                success_rate = 1.0 if completed else 0.0
                
                # Get other fields with defaults
                streak = int(item.get('current_streak', 0))
                priority = item.get('priority', 'NORMAL')
                habit_id = item.get('habit_id', 'unknown')
                
                # Initialize category if not exists
                if category not in categories:
                    categories[category] = {}
                
                # Group by habit_name instead of individual records
                if habit_name not in categories[category]:
                    categories[category][habit_name] = {
                        'name': habit_name,
                        'success_rates': [],
                        'priority': priority,
                        'streak': streak,
                ...
                # Add this record's completion to the habit's success rates
                categories[category][habit_name]['success_rates'].append(success_rate)
        ...
        for category, habits_dict in categories.items():
            if not habits_dict:
                continue
                
            habits_list = []
            for habit_name, habit_data in habits_dict.items():
                # Calculate average success rate for this habit
                success_rates = habit_data['success_rates']
                if success_rates:
            ...
            if habits_list:
                ...
                success_rates = [h['success_rate'] for h in habits_list]
                simple_avg = sum(success_rates) / len(success_rates)
                
                # Priority weights
                priority_weights = {'HIGH': 1.5, 'NORMAL': 1.0, 'LOW': 0.5}
                weighted_sum = sum(h['success_rate'] * priority_weights.get(h['priority'], 1.0) for h in habits_list)
                weight_total = sum(priority_weights.get(h['priority'], 1.0) for h in habits_list)
                weighted_avg = weighted_sum / weight_total if weight_total > 0 else simple_avg
                
                group_performance[category] = {
                    'habit_count': len(habits_list),
        ...
        print(f"🔍 Found {total_habits} processable habits from {len(items)} records")
        print(f"✅ Processed into {len(group_performance)} categories: {list(group_performance.keys())}")
        
        # If no habits found, return empty but valid structure
        if total_habits == 0:
            print("❌ No processable habit data found in DynamoDB")
            return {
                'group_performance': {},
                'summary': {
                    'total_groups': 0,
                    'total_habits_analyzed': 0,
                    'data_source': 'Real DynamoDB Data (No Habits Found)',
                    'habits_found': 0,
                    'last_sync': datetime.now().isoformat()
                },
                'last_updated': datetime.now().isoformat(),
                'status': 'success',
                'message': f'No processable habit data found in {len(items)} DynamoDB records'
            }
        
        print(f"✅ Successfully processed {total_habits} real habits from DynamoDB!")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Check if this is a habit details request
        path = event.get('path', '/api/groups')
        if '/habit/' in path:
            return handle_habit_details(event, headers)
        ...
        signal.alarm(10)  # 10 second timeout for DynamoDB
...
def handle_habit_details(event, headers):
    """Handle individual habit detail requests"""
    try:
        # Extract habit name from path: /api/habit/{habit_name}
        path = event.get('path', '')
        habit_name = path.split('/habit/')[-1] if '/habit/' in path else ''
        
        if not habit_name:
            return {
                'statusCode': 400,
                'headers': headers,
                'body': json.dumps({'error': 'Habit name required'})
            }
        
        # URL decode the habit name
        import urllib.parse
        habit_name = urllib.parse.unquote(habit_name)
        
        print(f"🔍 Fetching details for habit: {habit_name}")
        ...
        # Query for specific habit records
        response = table.scan(
            FilterExpression='habit_name = :habit_name',
            ExpressionAttributeValues={':habit_name': habit_name}
        )
        signal.alarm(0)
        
        items = response.get('Items', [])
        print(f"✅ Found {len(items)} records for habit: {habit_name}")
        ...
        daily_data = []
        for item in items:
            if item.get('SK', '').startswith('DAILY#'):
                daily_data.append({
                    'date': item.get('date', ''),
                    'completed': item.get('completed', False),
        ...
        completed_days = sum(1 for day in daily_data if day['completed'])
        success_rate = completed_days / total_days if total_days > 0 else 0
        
        habit_details = {
            'name': habit_name,
            'category': items[0].get('category', 'Uncategorized'),
            'priority': items[0].get('priority', 'NORMAL'),
        ...
        return {
            'statusCode': 200,
            'headers': headers,
            'body': json.dumps(habit_details, default=str)
        }
        
    except Exception as e:
        print(f"❌ Error in habit details: {str(e)}")
        return {
            'statusCode': 500,
            'headers': headers,
