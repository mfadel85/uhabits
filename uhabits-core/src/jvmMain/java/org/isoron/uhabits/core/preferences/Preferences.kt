...
open class Preferences(private val storage: Storage) {
    ...
    var dailyPerformanceSpinnerPosition: Int
        get() = min(2, max(0, storage.getInt("pref_daily_performance_spinner", 0)))
        set(position) {
            storage.putInt("pref_daily_performance_spinner", position)
        }
